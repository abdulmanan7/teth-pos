...
export default function StockAdjustmentsManager({ isDarkTheme = true, onClose }: StockAdjustmentsManagerProps) {
  ...
  const calculateAdjustmentSummary = (lines: AdjustmentLine[]) => {
    let totalIncrease = 0;
    let totalDecrease = 0;
    ...
    lines.forEach((line) => {
      if (line.difference > 0) {
        totalIncrease += line.difference;
        itemsIncreased++;
      } else if (line.difference < 0) {
        totalDecrease += Math.abs(line.difference);
        itemsDecreased++;
      }
    });

    return { totalIncrease, totalDecrease, itemsIncreased, itemsDecreased };
  };
                  ...
                  <div>
                    ...
                    {currentLine.product_id && (
                      <p className={`text-xs mt-1 ${
                        currentLine.adjusted_quantity < currentLine.current_quantity
                          ? 'text-red-400'
                          : currentLine.adjusted_quantity > currentLine.current_quantity
                          ? 'text-green-400'
                          : 'text-slate-400'
                        ...
                        {currentLine.adjusted_quantity < currentLine.current_quantity
                          ? `↓ Decrease by ${currentLine.current_quantity - currentLine.adjusted_quantity}`
                          : currentLine.adjusted_quantity > currentLine.current_quantity
                          ? `↑ Increase by ${currentLine.adjusted_quantity - currentLine.current_quantity}`
                          : 'No change'}
                      </p>
                    )}
                  </div>
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-slate-400">
                          {line.current_quantity} → {line.adjusted_quantity}
                          <span className={`ml-2 font-semibold ${
                            line.difference < 0 ? 'text-red-400' : line.difference > 0 ? 'text-green-400' : 'text-slate-400'
                          }`}>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredAdjustments.map((adjustment) => {
            const summary = calculateAdjustmentSummary(adjustment.lines);
            return (
              <div
                key={adjustment._id}
              ...
              >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span
                          className={`text-xs font-semibold text-white px-2.5 py-1 rounded-full ${getStatusColor(
                            adjustment.status
                          )}`}
                        >
                      ...
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        <div className="flex items-center gap-1.5">
                          <Calendar className={`w-4 h-4 ${isDarkTheme ? 'text-slate-400' : 'text-slate-500'}`} />
                          <span className={isDarkTheme ? 'text-slate-300' : 'text-slate-700'}>
                ...
                <div className="p-4">
                  {/* Reason Badge */}
                  <div className="mb-3">
                    <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full ${isDarkTheme ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' : 'bg-purple-100 text-purple-700 border border-purple-200'}`}>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-3">
                    <div className={`rounded-lg p-3 ${isDarkTheme ? 'bg-slate-600/30' : 'bg-slate-100'}`}>
                      <div className="flex items-center gap-2 mb-1">
                        <Package className={`w-4 h-4 ${isDarkTheme ? 'text-slate-400' : 'text-slate-500'}`} />
                        <span className={`text-xs font-medium ${isDarkTheme ? 'text-slate-400' : 'text-slate-600'}`}>Total Items</span>
                      </div>
                    ...
                    {summary.totalIncrease > 0 && (
                      <div className={`rounded-lg p-3 ${isDarkTheme ? 'bg-green-500/10 border border-green-500/30' : 'bg-green-50 border border-green-200'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={`text-xs font-medium ${isDarkTheme ? 'text-green-300' : 'text-green-700'}`}>Increased</span>
                        </div>
                        <p className="text-xl font-bold text-green-400">
                          +{summary.totalIncrease}
                        </p>
                        <p className={`text-xs ${isDarkTheme ? 'text-green-300/70' : 'text-green-600'}`}>
                          {summary.itemsIncreased} {summary.itemsIncreased === 1 ? 'item' : 'items'}
                        </p>
                      </div>
                    )}

                    {summary.totalDecrease > 0 && (
                      <div className={`rounded-lg p-3 ${isDarkTheme ? 'bg-red-500/10 border border-red-500/30' : 'bg-red-50 border border-red-200'}`}>
                        <div className="flex items-center gap-2 mb-1">
                          <TrendingDown className="w-4 h-4 text-red-400" />
                          <span className={`text-xs font-medium ${isDarkTheme ? 'text-red-300' : 'text-red-700'}`}>Decreased</span>
                        </div>
                        <p className="text-xl font-bold text-red-400">
                          -{summary.totalDecrease}
                        </p>
                        <p className={`text-xs ${isDarkTheme ? 'text-red-300/70' : 'text-red-600'}`}>
                          {summary.itemsDecreased} {summary.itemsDecreased === 1 ? 'item' : 'items'}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Notes Preview */}
                  {adjustment.notes && (
                    <div className={`text-sm p-2 rounded ${isDarkTheme ? 'bg-slate-600/20 text-slate-300' : 'bg-slate-50 text-slate-700'}`}>
                      <span className="font-medium">Note:</span> {adjustment.notes.length > 80 ? adjustment.notes.substring(0, 80) + '...' : adjustment.notes}
                    </div>
                  )}

                  {/* Approval Info */}
                  {adjustment.status === "approved" && adjustment.approved_by && (
                    <div className={`mt-3 flex items-center gap-2 text-xs ${isDarkTheme ? 'text-slate-400' : 'text-slate-600'}`}>
                      <User className="w-3.5 h-3.5" />
                      <span>Approved by {adjustment.approved_by}</span>
                      {adjustment.approved_date && (
                        <span>• {new Date(adjustment.approved_date).toLocaleDateString()}</span>
                      )}
                    </div>
                  )}
                </div>
