  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center p-8">
        <p className="text-slate-400">Loading goods receipts...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">Goods Receipts</h2>
        <Button
          onClick={() => {
            setShowForm(true);
            setSelectedPO(null);
      ...
      <div className="grid gap-4">
        {goodsReceipts.length === 0 ? (
          <div className="text-center py-8 bg-slate-800/50 rounded-lg border border-slate-700">
            <p className="text-slate-400">No goods receipts yet. Create one to get started!</p>
          </div>
        ) : (
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="text-lg font-semibold text-white">{gr.receipt_number}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm text-slate-400 mb-2">
                    <div>PO: <span className="text-white">{gr.po_number}</span></div>
                    <div>Received: <span className="text-white font-semibold">{gr.total_received}</span></div>
                    <div>Damaged: <span className="text-white">{gr.total_damaged}</span></div>
                    <div>Date: <span className="text-white">{new Date(gr.receipt_date).toLocaleDateString()}</span></div>
                  </div>

                  {/* Items Summary */}
                  <div className="bg-slate-700/50 rounded p-2 mb-2">
                    <p className="text-xs text-slate-400 mb-1">Items ({gr.items.length}):</p>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center p-4 z-50">
          <div className="bg-slate-800 rounded-lg border border-slate-700 shadow-xl max-w-3xl w-full max-h-[90vh] overflow-auto">
            <div className="flex items-center justify-between p-6 border-b border-slate-700 sticky top-0 bg-slate-800">
              <h3 className="text-xl font-bold text-white">Create Goods Receipt</h3>
              <button
                ...
                <div className="border-t border-slate-600 pt-4">
                  ...
                  <div className="space-y-4">
                    {formData.items.map((item, index) => (
                      <div key={index} className="bg-slate-700/50 p-4 rounded space-y-3">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div>
                            <label className="text-xs text-slate-400">Received Qty</label>
                            <Input
                              type="number"
                              min="0"
                          ...
                          <div>
                            <label className="text-xs text-slate-400">Damaged Qty</label>
                            <Input
                              type="number"
                              min="0"
                        ...
                        {/* Quality Notes */}
                        <div>
                          <label className="text-xs text-slate-400">Quality Notes</label>
                          <Input
                            type="text"
                        ...
                        {/* Lot Numbers */}
                        <div>
                          <label className="text-xs text-slate-400">Lot Numbers (comma-separated)</label>
                          <Input
                            type="text"
                        ...
                        {/* Serial Numbers */}
                        <div>
                          <label className="text-xs text-slate-400">Serial Numbers (comma-separated)</label>
                          <Input
                            type="text"
